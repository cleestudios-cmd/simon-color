...
function subscribeToLeaderboard() {
  ...
  if (!db) {
    if (globalEntries.length === 0) {
      showLeaderboardMessage('No scores yet — play to set the first record!');
    }
    return;
  }
      ...
      (error) => {
        ...
        if (globalEntries.length === 0) {
          showLeaderboardMessage(
            'Could not load global scores. Create Firestore DB and publish rules (see FIREBASE_SETUP.md).'
          );
        }
      }
    );
